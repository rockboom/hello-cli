...
class Creator{
    ...
    async installDepencies(){
        const select = await Inquirer.prompt([
            {
                type:'list',
                name:"installTool",
                message:"choose an install tool?",
                choices:['yarn','npm']
            }
        ]);
        let options = {
            cwd: path.resolve(process.cwd(), this.name),
            stdio: "inherit"
        }
        if (select.installTool === 'npm') {
            await execa('npm', ['install'], options);
        } else if (select.installTool === 'yarn') {
    ...
    async create(){ // 开始创建
        ...
        await this.installDepencies();
        console.log(chalk.blue(`cd ${this.name}`));
        console.log(chalk.blue('hello serve'));
    }
}
